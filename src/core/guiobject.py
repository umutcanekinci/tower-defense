...
class ImageObject():
	def __init__(self, path: str, pos: tuple[int, int], size: tuple[int, int] = (0, 0), extension: str = ".png") -> None:
		self.image = scale_surface(load_image(path, extension), size) if size != (0, 0) else load_image(path, extension)
		self.rect = self.image.get_rect(topleft = pos)

	def is_mouse_on(self, mouse_pos) -> bool:		
		return mouse_pos is not None and self.rect.collidepoint(mouse_pos)
...
class GuiObject(object):
	...
	def __init__(self, surface_size:  tuple[int, int] = (0, 0), pos = ("CENTER", "CENTER"), size =(None, None), image_path = None, image_path2 = None, extension1 = ".png", extension2 = ".png") -> None:
		...
		self.images[self.STATE.OVER] = ImageObject(image_path2, (x, y), size, extension2) if image_path2 is not None else self.images[self.STATE.NORMAL]
